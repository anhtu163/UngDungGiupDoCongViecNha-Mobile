...
export default function DetailShoppingList({navigation, route}) {
  // const [choose, setchoose] = useState(1);

  const dataDetail = route.params.data;
  const {socket} = route.params;
  // console.log(dataDetail);
  const [list, setlist] = useState(dataDetail.listItems);
  ...
  const [keyMember, setkeyMember] = useState(
    dataDetail.assign ? dataDetail.assign._id : null,
  );
  ...
  const [listBillImg, setlistBillImg] = useState(
    dataDetail.bill ? dataDetail.bill : [],
  );
  const [total, settotal] = useState(
    dataDetail.total ? dataDetail.total : null,
  );
  const [listType, setlistType] = useState(null);
  const [keyType, setkeyType] = useState(dataDetail.stID._id);

  //console.log(times);
  const [err, seterr] = useState('');
  const [repeat, setrepeat] = useState(
    dataDetail.repeat ? dataDetail.repeat.type : null,
  );
  const [showRepeat, setshowRepeat] = useState(
    dataDetail.repeat ? true : false,
  );
  const [times, settimes] = useState(
    dataDetail.repeat ? dataDetail.repeat.every : 1,
  );
  ...
  const [isReminder, setisReminder] = useState(
    dataDetail.repeat ? dataDetail.repeat.isRemindered : false,
  );
  ...
  useEffect(() => {
    if (dataDetail.repeat) {
      setstartdate(new Date(dataDetail.repeat.start));
      setstarttime(new Date(dataDetail.repeat.start));
    }
  }, []);
  useEffect(() => {
  ...
  // useEffect(() => {
  //   if (listType) {
  //     setkeyType(dataDetail.stID._id);
  //   }
  // }, [listType]);
  ...
  const handleEditList = () => {
    ...
    if (name === null || name === '') {
    ...
    } else if (showRepeat && !times) {
      seterr('Khoảng cách lặp phải là số');
      ...
      return RNFetchBlob.fetch(
        'POST',
        'https://househelperapp-api.herokuapp.com/edit-shopping-list',
        {
          Authorization: 'Bearer ' + route.params.token,
        ...
        <View style={{margin: 5}}>
              ...
              {showRepeat === true && (
                <View style={{marginBottom: 10}}>
                  ...
                  <InputItem
                    ...
                    value={times ? times.toString() : times}
                    style={{
                      fontSize: 16,
                      marginTop: 2,
                      minWidth: 10,
          ...
          <View style={{margin: 5}}>
            <Card style={{padding: 10}}>
              <Flex>
                <Icon name="credit-card" color="black" size="md" />
                <Text style={{color: '#0099FF', fontSize: 16, marginLeft: 5}}>
                  Thông tin hóa đơn:
                </Text>
              </Flex>
              ...
              <InputItem
                ...
                value={total ? total.toString() : total}
                style={{
                  fontSize: 16,
                  marginTop: 2,
                  minWidth: 10,
              ...
              <Flex justify="between" align="center" wrap="wrap">
                {listBillImg &&
                  listBillImg.map((item, index) => (
                    <TouchableOpacity
                      onPress={() =>
                        navigation.navigate('ImageBill', {img: item})
                      }>
                      <Image
                        resizeMode="cover"
                      ...
                      <TouchableOpacity
                        ...
                        <Icon name="close-circle" color="orange" size={20} />
                      </TouchableOpacity>
                    </TouchableOpacity>
                  ))}
                <View style={{margin: 10, marginLeft: 5, width: 50}}>
                  <TouchableOpacity onPress={pickImageHandler2}>
                    <Icon name="plus-circle" size="lg" color="#0099FF" />
                  </TouchableOpacity>
                </View>
              </Flex>
          ...
          <Button type="primary" style={{margin: 5}} onPress={handleEditList}>
