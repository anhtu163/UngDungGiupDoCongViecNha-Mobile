...
export default function EditTaskCategory({navigation, route}) {
  ...
  const HandleEditCategory = () => {
    const data = {
      tcID: route.params.id,
      name: name,
      image: image.uri,
    };
    RNFetchBlob.fetch(
      'POST',
      'https://househelperapp-api.herokuapp.com/edit-task-category',
      {
        Authorization: 'Bearer ' + token,
  ...
  const HandleDeleteCategory = () => {
    let tmpID;
    Modal.alert(
      'Chọn loại công việc thay thế',
      <Flex justify="between" wrap="wrap">
        {route.params.listCat
          .filter(item1 => item1._id !== route.params.id)
          .map(item => (
      ...
      [
        ...
        },
        {
          text: 'Chọn',
          onPress: () => {
            console.log('Loại công việc thay thế: ' + tmpID);
            const data = {
              tcIDDelete: route.params.id,
              tcIDReplace: tmpID,
            };
            RNFetchBlob.fetch(
              'POST',
              'https://househelperapp-api.herokuapp.com/delete-task-category',
              {
                Authorization: 'Bearer ' + token,
                'Content-Type': 'application/json',
              },
      ...
      <ScrollView>
        <View
          ...
          <View
            ...
            <InputItem
              clear
              placeholder={name || 'Tên loại công việc'}
              style={{fontSize: 18}}
              defaultValue={name}
